...
type EquivalenciaType =
  ...
  | "Militar"
  | "Autônomo Inscrito"
  | "Autônomo Não inscito"
  | "Empresário";

import { FileUpload } from "@/src/components/uploadArquivos";
...
export function CTPS() {
  const [formData, setFormData] = useState<FormState>({
    tipoEquivalencia: "CTPS", // Valor inicial da imagem
    upload: "",
  ...
  return (
    <form onSubmit={handleSubmit} className="flex bg-white ml-5">
      ...
      <div className="w-1/3 p-8 relative bg-black text-white rounded-l-lg equivalence-form-left-bg">
        <h3 className="text-xs text-white mb-4">TIPO DE EQUIVALÊNCIA</h3>
        ...
        <select
          ...
          className="w-full p-4 mb-8 bg-white text-black border border-gray-300 rounded-md shadow-sm appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-red-500"
        >
          <option value="CTPS">Solicitação CTPS</option>
          <option value="Militar">Solicitação Militar</option>
          <option value="Autônomo Inscrito">
            Solicitação Autônomo inscrito
          </option>
          <option value="Autônomo Não inscito">
            Solicitação Autônomo não inscrito
          </option>
          <option value="Empresário">Solicitação Empresário</option>
        </select>

        <h3 className="text-xs text-white mb-4">UPLOAD DE ARQUIVO</h3>
        <FileUpload
          label="Informativo CTPS"
          buttonText="Informativo CTPS"
          darkStyle={true} // Aplica o estilo preto
          accept="image/*, application/pdf, .doc, .docx, .txt" // Arquivos permitidos
          onFileUpload={(files) => {
            console.log("Arquivos para Informativo CTPS:", files);
            // Lógica para processar e salvar os arquivos no estado ou enviar para a API
          }}
        />
